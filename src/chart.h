...
public:
      ...
      int         start;
      int         end;
      wxString    decr_dll;         // like cryp0201.dll
      wxString    m_cert_file_name;
};

WX_DECLARE_OBJARRAY(ecr_region_descriptor, ArrayOfECR_Regions);




class Decrypted_FileInputStream;
...
//      bool RenderViewOnDC(wxMemoryDC& dc, const PlugIn_ViewPort& VPoint, int scale_type);
      ...
      bool ProcessECR3(Decrypted_FileInputStream *ifs_hdr);
      ...
      int ReadBSBHdrLine( Decrypted_FileInputStream*, char *, int );
...
//    Protected Data
      ...
      Decrypted_FileInputStream     *ifs_hdr;
...
protected:
      ...
      int               m_ecr3_begin;
      int               m_ecr_length;
      wxString          m_ecr3_cert;

      int               m_partial_line_length;        // number of bytes in partial lines stored in ECR3 region
      unsigned char     *m_ppartial_bytes;
